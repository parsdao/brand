...
Web Design Spec, sized to the Pars brand. Lives next to `LLM.md` —
`LLM.md` describes what's built, this file describes how it should look.

The canonical brand JSON for Pars is `brand.json` in this repo. That
...
### 1.1 Voice
Inclusive, multilingual, protocol-first. Equally valid in English and فارسی. Cultural references welcome when relevant.

### 1.2 Brand atom
Indigo-led, with Persian heritage cues. RTL-aware layouts. Single primary hue with hover variant; secondary surfaces tinted with the same indigo at low opacity.
...
| Token | Value | Use |
|---|---|---|
| `accent1` (light) | `#4F46E5` | Indigo-600 primary |
| `accent1Hovered` | `#4338CA` | Indigo-700 hover |
| `accent1` (dark) | `#818CF8` | Indigo-400 dark-mode primary |
| `surface1` (dark) | `#000000` | Page surface |
| `surface2` (dark) | `#1E1B4B` | Indigo-tinted card surface |
...
### 3.1 Family
Geist Sans for English, IRANSans / Vazirmatn for Persian (RTL).
...
primary app (pars.network). If a page reads visibly different from the
